...
async function roleUpdate(o,n,data){const c=[];if(o.name!==n.name){const d=diff(o.name,n.name);c.push("**Name**\nPrevious: "+d.old+"\nNew: "+d.next);}if(o.hexColor!==n.hexColor)c.push("**Color:** "+o.hexColor+" → "+n.hexColor);if(o.position!==n.position)c.push("**Position:** "+o.position+" → "+n.position);if(o.mentionable!==n.mentionable)c.push("**Mentionable:** "+(o.mentionable?"Yes":"No")+" → "+(n.mentionable?"Yes":"No"));if(!c.length)return;const a=await actor(n.guild,AuditLogEvent.RoleUpdate,n.id),e=base("roles","Role Updated","🎭 **Role:** "+n).addFields({name:"🔎 Changes",value:clip(c.join("\n\n"),3800)},{name:"🛡️ Changed By",value:a?detailsUser(a):"Unknown"});await send(n.guild,data,"roles",e);}
