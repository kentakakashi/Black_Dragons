...
async function hydrateSetupChannels(guild,data){
  ...
  for(const id of [h.channelId,r.registrationChannelId,r.reviewChannelId,r.historyChannelId,r.leaderboardChannelId,...Object.values(l)]){
...
async function handleSetupSelect(i,d){if(!i.customId.startsWith("setup_"))return false;const s=check(i);if(typeof s==="string"){await i.reply({content:s,ephemeral:true});return true}if(i.customId==="setup_category"){const k=i.values[0];await i.update({embeds:[createCategoryEmbed(view(d,s),k)],components:cat(k)});return true}if(i.customId.startsWith("setup_setting:")){const p=i.customId.split(":"),c=p[1],k=i.values[0],set=getSetting(c,k);if(!set){await i.reply({content:"❌ Unknown setup setting.",ephemeral:true});return true}await i.update({embeds:[createSettingEmbed(view(d,s),c,k,i.guild)],components:val(c,k,set)});return true}if(i.customId.startsWith("setup_value:")){const p=i.customId.split(":"),c=p[1],k=p[2],set=getSetting(c,k);if(!set){await i.reply({content:"❌ Unknown setup setting.",ephemeral:true});return true}apply(s.draft,k,i.values[0]);await i.update({embeds:[createCategoryEmbed(view(d,s),c,"📝 **"+set.label+" added to your draft.**")],components:cat(c)});return true}return false}
function apply(d,k,v){d.config ||= {};d.config.helpDesk ||= {};d.config.rank ||= {};d.config.rank.rankRoleIds ||= {};d.config.logs ||= {categoryId:null,channels:{}};d.config.logs.channels ||= {};if(k==="helpdesk_channel")d.config.helpDesk.channelId=v;else if(k==="war_role")d.config.helpDesk.warRoleId=v;else if(k==="backup_role")d.config.helpDesk.backupRoleId=v;else if(k==="rank_registration")d.config.rank.registrationChannelId=v;else if(k==="rank_review")d.config.rank.reviewChannelId=v;else if(k==="rank_history")d.config.rank.historyChannelId=v;else if(k==="leaderboard_channel")d.config.rank.leaderboardChannelId=v;else if(k.startsWith("rank_role_"))d.config.rank.rankRoleIds[k.slice(10)]=v;else if(k.startsWith("log_"))d.config.logs.channels[k.slice(4)]=v;d.rankConfig={...(d.rankConfig||{}),registrationChannelId:d.config.rank.registrationChannelId,reviewChannelId:d.config.rank.reviewChannelId,historyChannelId:d.config.rank.historyChannelId,rankRoleIds:{...((d.rankConfig||{}).rankRoleIds||{}),...(d.config.rank.rankRoleIds||{})}}}
